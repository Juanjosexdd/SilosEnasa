...
<x-card-body>
    ...
    <div class="row">
        ...
        {{-- <div class="col-md-3">
                {!! Form::label('requisicion_id', 'Requisicion Nro. : ', ['class' => 'text-blue']) !!}
                <div class="input-group">
                    {!! Form::select('requisicion_id', $requisicions, null, ['class' => 'form-control  select2' . ($errors->has('requisicion_id') ? ' is-invalid' : ''), 'data-live-search' => 'true', 'placeholder' => '']) !!} 
                    {!! $errors->first('requisicion_id', ' <div class="invalid-feedback text-center"><strong>:message</strong></div>') !!}
                </div>
        ...
        <div class="col-md-2">
            ...
            <select class="form-control select2" name="requisicion_id" id="requisicion_id" data-live-search="true"
                ...
                @foreach ($requisicions as $prod)
                    <option value="{{ $prod->id }}-{{ $prod->departamento }}">{{ $prod->correlativo }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-4">
            <label class="text-blue"  for="departamento">Solicitante :</label>
            <input type="text" disabled id="departamento" name="departamento" class="form-control"
...
<script>
    ...
    var cont = 0;
    // $("#guardar").hide();
    $("#requisicion_id").change(mostrarValores);

    function mostrarValores() {

        datosProducto = document.getElementById('requisicion_id').value.split('-');
        $("#departamento").val(datosProducto);
    ...
    function agregar() {
        ...
        datosProducto = document.getElementById('requisicion_id').value.split('_');
