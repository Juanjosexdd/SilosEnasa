<div class="card elevation-5 col-md-12 col-sm-12 pt-3" style="border-radius: 0.95rem" bis_skin_checked="1">
    <div class="card-header" style="padding: .75rem .25rem" bis_skin_checked="1">
        <div class="input-group mb-3">
            ...
            <a href="{{ route('admin.egresos.create') }}" class="btn bg-navy btn-sm px-2 elevation-4"><i class="fas fa-plus mt-2 px-3"></i>
            </a>
            @endcan
        </div>
    </div>
        ...
        <table class="table table-striped table-hover text-nowrap">
            <thead>
                <tr class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2">
                    ...
                    <th scope="col" role="button" class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7 ps-2" wire:click="order('empleado_id')">
                        Solicitante
                        @if ($sort == 'empleado_id')
                        @if ($direction == 'asc')
                        <i class="fas fas fa-sort-amount-down-alt float-right mt-1"></i>
            ...
            <tbody>
                ...
                <tr class="text-secondary text-sm font-weight-bold">
                    ...
                    <td>
                        @if($egreso->solicitud_id)
                        <a href="{{ route('admin.egresos.show', $egreso->id) }}">
                            {{ $egreso->solicitud->user->display_user }}
                        </a>
                        @endif
                        @if($egreso->empleado_id)
                        <a href="{{ route('admin.egresos.show', $egreso->id) }}">
                            {{ $egreso->empleado->display_empleado }}
                        </a>
                        @endif
                    </td>
                    <td>
                        @if($egreso->solicitud_id)
                        <a href="{{ route('admin.egresos.show', $egreso->id) }}">
                            {{ $egreso->solicitud->user->departamento->nombre }}
                        </a>
                        @endif
                        @if($egreso->empleado_id)
                        <a href="{{ route('admin.egresos.show', $egreso->id) }}">
                    ...
                    <td>
                        @if ($egreso->estatus == 1)
                        @can('admin.egresos.estatuegresos')
                        <form class="anular" action="{{ route('admin.egresos.estatuegreso', $egreso) }}" method="get">
                            @csrf
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <a class="btn btn-default elevation-4 btn-sm" style="border-color: rgb(158, 157, 157);" href=" {{ route('admin.egresos.show', $egreso->id) }} ">
