                ...
                <tbody>
                    @foreach ($biennacionals as $biennacional)
                        <tr class="text-secondary font-weight-bold text-sm">
                            <td> <a href="{{ route('admin.biennacionals.show', $biennacional) }}">
                                    {{ $biennacional->codigo }}</a></td>
                            {{-- <td>{{ $biennacional->id }}</td> --}}
                            <td><a href="{{ route('admin.biennacionals.show', $biennacional) }}">
                                    {{ $biennacional->nombre }}</a></td>
                            <td><a href="{{ route('admin.biennacionals.show', $biennacional) }}">
                                    {{ $biennacional->descripcion }} </a></td>
                            ...
                            <td>
                                ...
                                @elseif ($biennacional->estatus == 3)
                                    <span class="badge badge-secondary">Desincorporado</span>
                                @endif
                            </td>
                            <td>
                                @if ($biennacional->estatus == 1)
                                    <form class="formulario-eliminar"
                                        action="{{ route('admin.biennacionals.desincorporar', $biennacional) }}"
                                        method="get">
                                        @csrf
                                        <button type="submit" class="btn btn-success text-white btn-sm elevation-4">
                                            <i class="fas fa-check-circle"></i> Desincorporado &nbsp; &nbsp;
                                        </button>
                                    </form>

                                @elseif ($biennacional->estatus == 3)
                                    <button type="submit" disabled class="btn btn-secondary text-white btn-sm elevation-4">
                                        <i class="fas fa-check-circle"></i> Desincorporado
                                    </button>
                                @endif
                            </td>
                                            ...
                                            @elseif ($biennacional->estatus == 0)
                                                ...
                                                @elseif ($biennacional->estatus == 3)
                                                    <button type="submit" disabled
                                                        class="btn btn-default  text-secondary border-0 btn-sm p-0">
                                                        <i class="fas fa-times-circle"></i>
                                                    </button>
                                            @endif
                                        </a>
